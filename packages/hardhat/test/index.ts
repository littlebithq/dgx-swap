...
describe("DGX Swap Contract", function () {
  ...
  beforeEach("deploy contracts", async () => {
    ...
    const owner_wallet = await ethers.getSigner(
      "0xb779efeeda6cf887b80bc386e7eb9fdced6753f6"
    );
    ...
    await deployer.sendTransaction({
      to: cgtHolder.address,
      value: utils.parseEther("10"),
    });
    await deployer.sendTransaction({
      to: dgxHolder.address,
      value: utils.parseEther("10"),
    });

    await cacheGold
      .connect(owner_wallet)
      .setTransferFeeBasisPoints(cgtTransferFeeBP);
    await cacheGold
      .connect(cgtHolder)
      .transfer(
        swapContract.address,
        1000 * 10 ** (await cacheGold.decimals())
      );
  });
  ...
  it("Should allow user to swap DGX and receive equal amount of CGT", async function () {
    ...
    const initialCGTBalance = await cacheGold.balanceOfNoFees(
      dgxHolder.address
    );
    ...
    await dgxToken
      ...
      .approve(
        swapContract.address,
        amount * 10 ** (await dgxToken.decimals())
      );
    await swapContract
      .connect(dgxHolder)
      .swap(amount * 10 ** (await dgxToken.decimals()));
    ...
    expect(
      (initialDGXBalance.toNumber() - finalDGXBalance.toNumber()) /
        10 ** (await dgxToken.decimals())
    ).to.be.equal(amount);
    expect(
      (finalCGTBalance.toNumber() - initialCGTBalance.toNumber()) /
        10 ** (await cacheGold.decimals())
    ).to.be.equal(amount);
    expect(
      (finalAmountBurnt.toNumber() - initialAmountBurnt.toNumber()) /
        10 ** (await dgxToken.decimals())
    ).to.be.equal(amount);
    console.log(
      "CGT fees: ",
      amount -
        (await cacheGold.balanceOf(dgxHolder.address)).toNumber() /
          10 ** (await cacheGold.decimals())
    );
  });
  ...
  it("Should not allow user to swap DGX without approving", async function () {
    await expect(
      swapContract
        .connect(dgxHolder)
        .swap(100 * 10 ** (await dgxToken.decimals()))
  ...
  it("Should not allow user to swap DGX without sufficient balance", async function () {
    await dgxToken.approve(
      swapContract.address,
      100 * 10 ** (await dgxToken.decimals())
    );
    await expect(
  ...
  it("Should not allow user to swap if contract has insufficient CGT balance", async function () {
    await dgxToken
      .connect(dgxHolder)
      .approve(swapContract.address, 1000 * 10 ** (await dgxToken.decimals()));
    await expect(
      swapContract
        .connect(dgxHolder)
        .swap(1000 * 10 ** (await dgxToken.decimals()))
  ...
  it("Should correctly round off DGX amount while swapping", async function () {
    ...
    expect(
      finalCGTBalance.toNumber() - initialCGTBalance.toNumber()
    ).to.be.equal(Math.round(amount / 10));
